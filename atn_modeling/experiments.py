...
from atn_modeling.atn_predictor_instances import ATN_PREDICTORS, ATN_PREDICTORS_PLUS_CSF, ATN_PREDICTORS_PLUS_PLASMA
...
def experiment_combo_atn_vs_baseline_CSF(dataset, target,
                                         covariates=['Age', 'SexBinary', 'HasE4Binary'],
                                         stratify='CDRBinned',
                                         repeats=10,
                                         outer_splits=10,
    ...
    for r in range(repeats):
        ...
        for i, (outer_train_index, outer_test_index) in enumerate(outer_cv.split(dataset, dataset[stratify])):
            ...
            for j, (inner_train_index, inner_test_index) in enumerate(inner_cv.split(outer_train, outer_train[stratify])):
                ...
                # testing many ATN models
                for biomarker, variable_dict in ATN_PREDICTORS_PLUS_CSF.items():
                    for variable_type, model_list in variable_dict.items():
                        for model in model_list:
                            metrics, _ = test_atn_linear_model(models=model,
                                                               covariates=covariates,
            ...
            # develop combinations
            FINAL_ATN_MODELS = {
                'Baseline': get_combo_atn_model(lm_selected_models, ATN_PREDICTORS_PLUS_CSF, None, None, None),
                'All binary [Imaging]': get_combo_atn_model(lm_selected_models, ATN_PREDICTORS_PLUS_CSF, 'binary', 'binary', 'binary'),
                'All binary [CSF]': get_combo_atn_model(lm_selected_models, ATN_PREDICTORS_PLUS_CSF, 'binary', 'binary', 'binary', csf=True),
                'All categorical [Imaging]': get_combo_atn_model(lm_selected_models, ATN_PREDICTORS_PLUS_CSF, 'categorical', 'categorical', 'categorical'),
                'All categorical [CSF]': get_combo_atn_model(lm_selected_models, ATN_PREDICTORS_PLUS_CSF, 'categorical', 'categorical', 'categorical', csf=True),
                'All continuous [Imaging]': get_combo_atn_model(lm_selected_models, ATN_PREDICTORS_PLUS_CSF, 'continuous', 'continuous', 'continuous'),
                'All continuous [CSF]': get_combo_atn_model(lm_selected_models, ATN_PREDICTORS_PLUS_CSF, 'continuous', 'continuous', 'continuous', csf=True),
                }
...
def experiment_combo_atn_vs_baseline_PLASMA(dataset, target,
                                            covariates=['Age', 'SexBinary', 'HasE4Binary'],
                                            stratify='CDRBinned',
                                            repeats=10,
                                            outer_splits=10,
    ...
    for r in range(repeats):
        ...
        for i, (outer_train_index, outer_test_index) in enumerate(outer_cv.split(dataset, dataset[stratify])):
            ...
            for j, (inner_train_index, inner_test_index) in enumerate(inner_cv.split(outer_train, outer_train[stratify])):
                ...
                # testing many ATN models
                for biomarker, variable_dict in ATN_PREDICTORS_PLUS_PLASMA.items():
                    for variable_type, model_list in variable_dict.items():
                        for model in model_list:
                            metrics, _ = test_atn_linear_model(models=model,
                                                               covariates=covariates,
            ...
            # develop combinations
            FINAL_ATN_MODELS = {
                'Baseline': get_combo_atn_model(lm_selected_models, ATN_PREDICTORS_PLUS_PLASMA, None, None, None),
                'All binary [Imaging]': get_combo_atn_model(lm_selected_models, ATN_PREDICTORS_PLUS_PLASMA, 'binary', 'binary', 'binary'),
                'All binary [Plasma]': get_combo_atn_model(lm_selected_models, ATN_PREDICTORS_PLUS_PLASMA, 'binary', 'binary', 'binary', plasma=True),
                'All categorical [Imaging]': get_combo_atn_model(lm_selected_models, ATN_PREDICTORS_PLUS_PLASMA, 'categorical', 'categorical', 'categorical'),
                'All categorical [Plasma]': get_combo_atn_model(lm_selected_models, ATN_PREDICTORS_PLUS_PLASMA, 'categorical', 'categorical', 'categorical', plasma=True),
                'All continuous [Imaging]': get_combo_atn_model(lm_selected_models, ATN_PREDICTORS_PLUS_PLASMA, 'continuous', 'continuous', 'continuous'),
                'All continuous [Plasma]': get_combo_atn_model(lm_selected_models, ATN_PREDICTORS_PLUS_PLASMA, 'continuous', 'continuous', 'continuous', plasma=True),
                }
