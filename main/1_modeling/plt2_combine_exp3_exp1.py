...
def main():
    ...
    exp3_folder = os.path.abspath(os.path.join(this_dir, '..', '..', 'outputs', 'exp3_combo_atn_models_global_cognition_vs_binary'))
    exp1_folder = os.path.abspath(os.path.join(this_dir, '..', '..', 'outputs', 'exp1_svms_global_cognition'))
    
    exp3_results = os.path.join(exp3_folder, 'results.csv')
    exp1_results = os.path.join(exp1_folder, 'results.csv')
    
    if not os.path.exists(exp3_results):
        raise RuntimeError('Results for exp3 are missing.')
