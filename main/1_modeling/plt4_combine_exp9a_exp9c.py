...
def main():
    ...
    expA_folder = os.path.abspath(os.path.join(this_dir, '..', '..', 'outputs', 'exp9a_preclinical_combo_atn_models'))
    expC_folder = os.path.abspath(os.path.join(this_dir, '..', '..', 'outputs', 'exp9c_preclinical_svms'))
    
    expA_results = os.path.join(expA_folder, 'results.csv')
    expC_results = os.path.join(expC_folder, 'results.csv')
    
    if not os.path.exists(expA_results):
        raise RuntimeError('Results for exp9a are missing.')
    ...
    # concatenate data
    expA = pd.read_csv(expA_results)
    expC = pd.read_csv(expC_results)
    concat = pd.concat([expA, expC])
